...
export default function CohortsPage() {
  ...
  useEffect(() => {
    async function loadCohorts() {
      try {
        const response = await fetch("/api/events/category/Cohorts");
      ...
      <div className="relative bg-black/40 py-6 md:py-10">
        <div className="mx-auto max-w-7xl px-4 text-center">
          <h1 className="font-display text-5xl font-bold tracking-wide text-white md:text-7xl mb-2 md:mb-4">
            Learning <span className="text-electric-purple">Cohorts</span>
          </h1>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl bg-white/5">
            ...
            <p className="text-gray-400">
              Join the waitlist for upcoming batches.
            </p>
          </div>
        ) : (
